...
POP_SIZE =200# int(input('Insert POP_SIZE')) #size of population
P_CROS = 0.6#float(input('Insert P_CROS')) #probability for crossover 
P_MUT = 0.01#float(input('Insert P_MUT')) #probability for mutation
MAX_GEN= 50#int(input('Insert MAX_GEN')) #max number of generations
...
def main(user_id):
    ...
    return np.mean(rmse_error_list),np.mean(mae_error_list)
############################################################################
def results(user_id):
   averages=[]
   gens_av=[]
...
if __name__ == "__main__":
   choice=int(input('Count error for 50 users? \n 1.No \n 2.Yes \n'))
   if choice == 1:
      user_id = 269 #int(input('Insert user id'))
      setting(user_id)
      results(user_id)
   elif choice == 2:
      total_rmse_error_list=[]
      total_mae_error_list=[]
      for u in range(10):
         rand_user_id = random.randint(1,943) #this is for 50 users computation of errors
         user_id =rand_user_id
         setting(user_id)
         print('Setting done')
         #best,gens,rmse_error,mae_error = main(user_id)
         rmse_error,mae_error = main(user_id)
         print('rmse:',rmse_error)
         print('mae:',mae_error)
         total_rmse_error_list.append(rmse_error)
         total_mae_error_list.append(mae_error)
         print('user:',u+1,'/50 done')
      RMSE_AVERAGE=np.mean(total_rmse_error_list)
      MAE_AVERAGE=np.mean(total_mae_error_list)
      print('RMSE AVERAGE & MAE AVERAGE')
      print(RMSE_AVERAGE,MAE_AVERAGE)
